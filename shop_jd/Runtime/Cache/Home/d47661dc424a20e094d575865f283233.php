...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="/shop_jd/Home/Public/css/verifyMail.css" media="all">
              ...
              <dd class="fore1_5" id="_MYJD_comments">
				...
				<div id="main">
						...
						<div class="mc">
							...
							<div class="form">
								<div class="item">
									...
									<div class="fl">
										<input  value=""  tabindex="1" class="itxt" name="email" id="email" type="text">
										<div class="clr"></div>
										<div id="email_text" class="msg-text" style="display: none;">请输入您的常用邮箱</div>
										<div class="clr"></div>
										<div id="email_error" class="msg-error" style="display: none;"></div>
									</div>
									<div class="clr"></div>
								</div>
								...
								<div class="item">
									...
									<div class="fl">
										<a id="sendMail" class="btn-9" href="javascript:void(0)">发送验证邮件</a>
									</div>
									<div class="clr"></div>
								</div>
							</div>
					...
					<div class="mod-main mod-comm">
						...
						<div class="mc">
							<div>1. 验证邮箱可加强账户安全，您可以使用已验证邮箱快速找回密码或支付密码；</div>
							<div>2. 已验证邮箱可用于账户余额变动提醒。</div>
						</div>
					</div>
				</div>
			</div>
			...
			<div id="footer-2014">
				...
				<div class="authentication">
					...
					<a rel="nofollow" target="_top" href="https://search.szfw.org/cert/l/CX20120111001803001836">
						<img src="54b8875fnad1e0c4c.png" class="/shop_jd/Home/Public/images/err-product" height="32" width="103"></a>
					<a target="_top" href="http://jubao.china.cn:13225/reportform.do">
						<img src="54b8863dn8d2c61ec.png" class="/shop_jd/Home/Public/images/err-product" height="32" width="185"></a>
				</div>
			</div>
		</div>
	</div>
...
<script>
  ...
  $(function () {
    ...
    $('#email').blur(function () {
      var val = $(this).val(); //获取表单值
      if(val.length>0){//有输入值
        if(!checkEmail(val)){//邮箱格式不正确
          $('#email_error').text('邮箱格式不正确，请重新输入').show();
          $('#email_text').hide();
          $('#email').attr('class','itxt');
          error = 1;
        }else{//格式正确
          $('#email').attr('class','itxt');
          $('#email_text').hide();
    ...
    $('#sendMail').click(function(){
      if(!error){
        ...
        var $email = $('#email').val();
        $.ajax({
          url:'/index.php/Home/Member/setMail',
          type:'post',
          data:{'uid':"<?php echo ($_SESSION['login_info']['uid']); ?>",'email':$email},
          success:function($msg){
            if($msg=='1'){
              window.location.href = "/index.php/Home/Member/sendMailSuccess/uid/<?php echo ($_SESSION['login_info']['uid']); ?>/email/"+$email;
            }else{
              alert('邮件验证失败')
            }
        }
