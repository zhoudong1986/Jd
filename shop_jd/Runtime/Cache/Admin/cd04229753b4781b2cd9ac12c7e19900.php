      ...
      <?php if(is_array($admins)): $i = 0; $__LIST__ = $admins;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ad): $mod = ($i % 2 );++$i;?><tr>
          <td scope="row">
            ...
            <td>
              ...
              <?php elseif($ad['role'] == 2): ?>管理员<?php echo (session('role')); ?>
              <?php else: ?>网络编辑<?php endif; ?>
                </td>
                <td>
                  <button class="btn <?php echo ($ad['right']); ?> btn-xs"><span class="<?php echo ($ad['status']); ?>"></span></button></td>
                  <td><?php echo ($ad['login_time']); ?></td>
                  <td>
                    <div>
                       <a href="/Jd/index.php/Admin/Member/edit/id/<?php echo ($ad['admin_id']); ?>" class="btn btn-info btn-xs">
                      <span class="glyphicon glyphicon-pencil"></span>
                      编辑
                    </a>
                    <?php if($_SESSION['role']< $ad['role']): ?><a href=""  admin_id="<?php echo ($ad['admin_id']); ?>" class="btn btn-success btn-xs">
                      <span><?php echo ($ad['button']); ?></span>
                    </a><?php endif; ?>
                    <?php if(($ad['role'] != 1) AND ($_SESSION['role']== 1)): ?><a href="/Jd/index.php/Admin/Member/del/id/<?php echo ($ad['admin_id']); ?>" class="btn btn-danger btn-xs">删除</a><?php endif; ?>
                    </div>
