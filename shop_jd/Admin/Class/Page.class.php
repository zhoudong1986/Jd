	 ...
	 */  
	 ...
	 public function get_page(){ 
		...
		if(self::$lastnum>0){
		 ...
		 $totalrow = '<span class="t_st_out" style="color:#003399;border-color:#ccc">'.sprintf('%d/%d页&nbsp;共%d条记录',isset($this->pagenum)?$this->pagenum+1:1,self::$lastnum+1,self::$totalrow).'</span>';
		 return '<style>.t_st_in{border:1px solid #ccc;margin:2px;padding:5px 10px 5px 10px;font-size:14px;background:white;color:#003399;text-align:center;'.$other_style.'}.t_st_in:hover{cursor:pointer;text-decoration:none;}.t_st_in:hover{border:1px solid #f30;color:#f30}.just_st_index{border:1px solid #666;background:#666;color:#fff;margin:2px;padding:5px 10px 5px 10px;font-size:14px;font-weight:bold} a{text-decoration:none;}.t_st_out{border:1px solid #EBEBEB;margin:2px;padding:5px 10px 5px 10px;font-size:14px;color:#ccc;background:white;text-align:center;'.$other_style.'}</style>'. $naviga . $this->get_pages() .$next.$totalrow.$ajax_fun;
		  }
		 }    
  }
